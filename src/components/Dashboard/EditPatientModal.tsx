...
const EditPatientModal: React.FC<EditPatientModalProps> = ({
  ...
  // Pre-fill form data when patient data is available
  useEffect(() => {
    if (patient && open) {
      ...
      setFormData({
        givenName: nameObj.given?.[0] || "",
        familyName: nameObj.family || "",
        gender: patient.gender || "",
        birthDate: patient.birthDate || "",
        identifier: identifier?.value || "",
        identifierUse: identifier?.use || "usual",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await fhirApi.put(`/Patient/${patient.id}`, patientResource);
      onSuccess();
    } catch (err: any) {
      setError(
